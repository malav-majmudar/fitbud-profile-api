...
router.patch("/:diaryId", async (request, response) => {
  ...
  try {
    if (String(diary.userId) !== request.body.userId) {
      ...
      return response
        .status(400)
        .send({ message: "You have ran out of logs!" });
    } else {
      if (request.body.type === "food") {
        ...
        if (request.body.action === "addLog") {
          ...
          diary[request.body.contents.mealPosition].foodLogs.splice(
            request.body.contents.logPosition,
            1
          );
          diary.numLogs = diary.numLogs + 1;
        ...
        if (request.body.action === "addLog") {
          ...
          diary[request.body.contents.mealPosition].recipeLogs.push(
            tempRecipeLog
          );
          diary.numLogs = diary.numLogs - 1;
          console.log("recipe log added");
        } else if (request.body.action === "deleteLog") {
          diary[request.body.contents.mealPosition].recipeLogs.splice(
            request.body.contents.logPosition,
            1
          );
          diary.numLogs = diary.numLogs + 1;
        ...
        if (request.body.action === "addLog") {
          ...
          diary["exercise"].strengthLogs.splice(
            request.body.contents.logPosition,
            1
          );
          diary.numLogs = diary.numLogs + 1;
        ...
        if (request.body.action === "addLog") {
          ...
          diary["exercise"].cardioLogs.splice(
            request.body.contents.logPosition,
            1
          );
          diary.numLogs = diary.numLogs + 1;
        ...
        if (request.body.action === "addLog") {
          ...
          console.log(tempWorkoutLog);
          diary["exercise"].workoutLogs.splice(
            request.body.logPosition,
            1,
            tempWorkoutLog
          );
